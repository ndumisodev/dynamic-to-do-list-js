...
// has been fully loaded and parsed by the browser. This prevents errors that
// might occur if the script tries to access HTML elements before they exist.
document.addEventListener('DOMContentLoaded', function() {

    // Step 2: Select DOM Elements
    // Use document.getElementById to get references to the necessary HTML elements.
    // These constants will be used throughout the script to interact with the DOM.
    ...
    function addTask() {
        ...
        // Set the text content of the <li> to the task text entered by the user.
    ...
    // The instruction "Invoke the addTask function on DOMContentLoaded" seems to be
    // a copy-paste error from a previous task (fetching data) as it would cause
    // an empty task to be added immediately on page load, which is not logical for a To-Do list.
    // Therefore, `addTask()` is NOT directly invoked here.
    // Instead, the event listeners that trigger `addTask` are set up within DOMContentLoaded.
});